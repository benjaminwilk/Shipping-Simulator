...
//		do{
	...
	private int parseShoreMenu(Boat playerObject, int userDecision){
		...
		shoreMenu.put(2, () -> new Crewmen(playerObject));
		shoreMenu.put(3, () -> ShipStatusSubmenu(playerObject));//UpgradeShip(playerObject)); // Jumps to Checking ship Status.  Will eventually revamp this.
		shoreMenu.put(4, () -> shipFuel(playerObject));//new ShipStatus(playerObject)); // Jumps to refueling the ship.  Will eventually revamp this.
		shoreMenu.put(5, () -> PriceDisplay(playerObject));
		shoreMenu.put(6, () -> new SaveLoad(playerObject));
